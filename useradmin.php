<?php
// ============================================================================
//  임시 사용자 관리 도구 (TEMPORARY)
//  - MIGRATE_TOKEN(.env)으로 보호됩니다.
//  - 사용:  https://도메인/dronemission/useradmin.php?token=(MIGRATE_TOKEN)
//  - 비밀번호 재설정이 끝나면 이 파일(useradmin.php)을 반드시 삭제하세요.
// ============================================================================
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        if ($action === 'reset') {
            ...
            $allowed = ['admin', 'leader', 'operator', 'fire_coord', 'observer'];
            ...
            if ($login === '' || $pw === '') {
                ...
                $st->execute([$login, $hash, $role, $login]);
...
$T = h($token);
$roleKo = ['admin'=>'관리자','leader'=>'팀장','operator'=>'운용자','fire_coord'=>'화력협조','observer'=>'참관'];
?>
<!DOCTYPE html>
<html lang="ko">
<head>
...
<style>
  body{font-family:-apple-system,"Malgun Gothic","Noto Sans KR",sans-serif;background:#0f1a2b;color:#dce8f5;
    max-width:620px;margin:0 auto;padding:18px 16px 60px}
  ...
  input,select{width:100%;box-sizing:border-box;background:#0b1626;border:1px solid #26384f;border-radius:8px;
    color:#eaf2fc;padding:10px;font-size:15px}
  button{width:100%;margin-top:12px;border:none;border-radius:9px;padding:12px;font-size:15px;font-weight:700;
    background:linear-gradient(180deg,#e8a33d,#c9871b);color:#1a1206;cursor:pointer}
...
<body>
  ...
  <div class="warn">⚠ <b>임시 도구입니다.</b> 비밀번호 설정이 끝나면 서버에서 <code>useradmin.php</code> 파일을 <b>반드시 삭제</b>하세요. (HTTPS가 아니면 비밀번호가 평문으로 전송되니 주의)</div>

  <?php if ($msg): ?>
    <div class="msg <?= $ok ? 'ok' : 'warn' ?>"><?= h($msg) ?></div>
  <?php endif; ?>
  ...
  <div class="card">
    ...
    <table>
      ...
      <?php foreach ($users as $u): ?>
        <tr>
          <td><?= h($u['id']) ?></td>
          <td><code><?= h($u['login_id']) ?></code></td>
          <td><?= h($roleKo[$u['role']] ?? $u['role']) ?></td>
          <td><?= h($u['team_id'] ?? '-') ?></td>
        </tr>
      <?php endforeach; ?>
    </table>
  ...
  <div class="card">
    ...
    <form method="post">
      ...
      <select name="login_id">
        <?php foreach ($users as $u): ?>
          <option value="<?= h($u['login_id']) ?>"><?= h($u['login_id']) ?> (<?= h($roleKo[$u['role']] ?? $u['role']) ?>)</option>
        <?php endforeach; ?>
      </select>
      <label>새 비밀번호</label>
